...
public class DienThoaiRepository implements IDienThoaiRepository {
    ...
    public static void main(String[] args) {
        System.out.println(getTCS("DT04"));
    }

    @Override
    public DienThoai getOne(String ma) {
        try ( Session session = new HibernatUtil().getFACTORY().openSession()) {
            Query q = session.createQuery("FROM DienThoai WHERE maDienThoai =:ma");
    ...
    public static DienThoai getTCS(String ma) {
        try ( Session session = new HibernatUtil().getFACTORY().openSession()) {
            Query q = session.createNativeQuery("SELECT d.* FROM dbo.DienThoai d WHERE d.MaDienThoai =:a OR d.IdDienThoai =\n"
                    + "(SELECT IdDienThoai FROM dbo.DienThoaiNCC JOIN dbo.Imei i ON i.IdDienThoaiNCC = DienThoaiNCC.IdDTNCC WHERE i.MaImei =:a )",DienThoai.class);
            q.setParameter("a", ma);
            DienThoai dt = (DienThoai) q.getSingleResult();
            return dt;
        } catch (Exception e) {
